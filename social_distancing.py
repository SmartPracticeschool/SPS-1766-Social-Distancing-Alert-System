...
#Go to this MODEL_PATH & Fetch the names
...
if config.USE_GPU:
    #set CUDA as preferable backend target
...
vs = cv2.VideoCapture(r"pedastrian.mp4" if "pedastrian.mp4" else 0)
writer = None
...
while True:
    ...
    #resize the frame and then detect people (and only people)
    frame = imutils.resize(frame, width = 700)
    results = detect_people(frame,net,ln,personIdx=LABELS.index("person"))
    
    # initiate the set of indexes that violate the minimum social distance
    violate = set()
    ...
    if r"social-distance-ditector" != "" and writer is None:
